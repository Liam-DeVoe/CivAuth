...
from civauth import civ, json_response
...
def verify_code() -> Response:
    ...
    if app.store.join_failures_since(address, now - constants.JOIN_FAILURE_WINDOW) >= constants.JOIN_FAILURES:
...
def issue() -> Response:
    ...
    if secret is None or not auth.startswith("Bearer ") or not secrets.compare_digest(secret, auth[7:]):
    ...
    if not re.fullmatch(r"[0-9a-f]{32}", uuid) or not re.fullmatch(r"\w{1,16}", name, re.ASCII):
        return json_response({"error": "invalid_profile"}, 400)
    now = app.now()
    app.store.purge(now, constants.REQUEST_TTL, constants.JOIN_CODE_TTL, constants.JOIN_FAILURE_WINDOW)
    for _ in range(5):
